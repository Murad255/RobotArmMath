...
if(showFlag == 1)
    ...
    q=[90;90;90;0;1;0];
    Q=q*pi/180; % перевод в радианы
    
    [x,y,z,A,sign_hand, sign_elbow, or_ind]=robot_arm_I([0;-pi/2;pi/2;0;45*pi/180;0],showFlag);
    [Q_out]= inverceKinematicI(x,y,z,A, sign_hand, sign_elbow, or_ind);
    
    [x,y,z,A,sign_hand, sign_elbow, or_ind]=robot_arm_I(Q_out,showFlag);
    Q_out*180/pi
    
    %вращение второй подвижностью
    count =25; 
    for k =0:count
        qk=pi*2* k/count; %сделать оборот 
        [x,y,z,A]=robot_arm([0;-pi/2+qk;pi/2;0;45*pi/180;0],showFlag); 
        
    end
    
    %вращение второй подвижностью с инвертированным флагом sign_hand
    for k =0:count
        qk=pi*2*k/count; 
         cla
            [x,y,z,A,sign_hand, sign_elbow, or_ind]=robot_arm_I([0;-pi/2+qk;pi/2;0;45*pi/180;0],0);
            [Q_out]= inverceKinematicI(x,y,z,A, -sign_hand, sign_elbow, or_ind);
            [x,y,z,A]=robot_arm(Q_out,showFlag);
        drawnow
